"""Guards for Pydantic v2-compatible app source."""
    ...
    for path in _python_sources():
        for line_number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
            ...
            if regex.search(line):
                hits.append(f"{path.relative_to(APP_DIR.parent)}:{line_number}: {stripped}")
    return hits


def test_app_source_has_no_pydantic_v1_validator_api() -> None:
    hits = _source_hits(r"@\s*validator\b|\bfrom\s+pydantic\s+import\s+.*\bvalidator\b")
