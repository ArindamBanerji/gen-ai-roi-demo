...
def main() -> None:
    ...
    if r is not None and r.status_code == 200:
        ...
        _check("Alert queue", len(alerts) >= 3, f"count={len(alerts)}")
    else:
        alerts = []
        _check("Alert queue", False, f"status={r.status_code if r else 'TIMEOUT'}")
    ...
    if decision_id:
        r = call("POST", "/api/alert/outcome",
                 json={"alert_id": alert_id,
                       ...
                       "outcome": "correct"})
        if r is not None:
            _check("Outcome feedback accepted",
                 r.status_code == 200,
    ...
    if r is not None and r.status_code == 200:
        hist = r.json()
        _check("GAE /history endpoint (200 + valid structure)",
             "history" in hist and "total" in hist,
             f"total={hist.get('total')}  message={str(hist.get('message',''))[:50]}")
    else:
        _check("GAE /history endpoint (200 + valid structure)", False,
             f"status={r.status_code if r else 'TIMEOUT'}")
    ...
    if r is not None and r.status_code == 200:
        conv = r.json()
        _check("GAE /convergence endpoint",
        ...
        conv = {}
        _check("GAE /convergence endpoint", False,
             f"status={r.status_code if r else 'TIMEOUT'}")
    ...
    if r is not None and r.status_code == 200:
        comp = r.json()
        _check("Compounding metrics",
             "weekly_trend" in comp,
             f"weeks={len(comp.get('weekly_trend', []))}")
    else:
        _check("Compounding metrics", False,
             f"status={r.status_code if r else 'TIMEOUT'}")
