...
def test_stop_stale_backend_succeeds_when_port_already_clear(monkeypatch):
    runner = load_runner()

    monkeypatch.setattr(runner, "is_port_open", lambda _port: False)
    monkeypatch.setattr(
        runner.subprocess,
        "run",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(AssertionError("cleanup should not run")),
    )

    result = runner.stop_stale_backend(8001)

    assert result["initial_port_open"] is False
    assert result["final_port_open"] is False
    assert result["cleanup_attempted"] is False


def test_stop_stale_backend_succeeds_when_cleanup_nonzero_but_port_clear(monkeypatch):
    runner = load_runner()
    ...
    monkeypatch.setattr(runner, "is_port_open", lambda _port: next(states))
    monkeypatch.setattr(runner.subprocess, "run", lambda command, **_kwargs: commands.append(command) or Completed())

    result = runner.stop_stale_backend(8001)

    assert result["initial_port_open"] is True
    assert result["final_port_open"] is False
    assert result["cleanup_returncode"] == 1
    assert "$PID" not in commands[0][-1]
    assert "$ownerProcessId" in commands[0][-1]
    assert "if (-not $ownerProcessIds) { exit 0 }" in commands[0][-1]


def test_stop_stale_backend_powershell_has_no_owner_success_guard(monkeypatch):
    runner = load_runner()

    class Completed:
        returncode = 0
        stdout = "ownerProcessIds=\n"
        stderr = ""
    ...
    script = commands[0][-1]
    assert result["final_port_open"] is False
    assert "if (-not $ownerProcessIds) { exit 0 }" in script
    assert "$PID" not in script
    assert "$pid" not in script


def test_stop_stale_backend_succeeds_when_owner_process_disappears(monkeypatch):
    runner = load_runner()

    class Completed:
        returncode = 1
        stdout = "ownerProcessIds=1234\n"
    ...
    assert result["final_port_open"] is False
    assert result["owner_process_ids"] == ["1234"]
    assert result["cleanup_returncode"] == 1


def test_stop_stale_backend_fails_when_port_remains_open(monkeypatch):
    runner = load_runner()
    ...
    monkeypatch.setattr(runner.os, "name", "nt")
    monkeypatch.setattr(runner, "is_port_open", lambda _port: True)
    monkeypatch.setattr(runner.time, "sleep", lambda _seconds: None)
    monkeypatch.setattr(runner.subprocess, "run", lambda *_args, **_kwargs: Completed())

    with pytest.raises(runner.RouteValidationError) as exc:
        runner.stop_stale_backend(8001, timeout_seconds=0.01, poll_interval_seconds=0.001)

    message = str(exc.value)
    assert "failed to clear stale backend on port 8001" in message
    assert "final_port_open=True" in message
    assert "owner_process_ids=['1234']" in message
    assert "cleanup_returncode=1" in message


def test_false_baseline_does_not_fail_cleanup_when_port_clear(tmp_path, monkeypatch):
    runner = load_runner()
    phase = runner.PhaseConfig("false_baseline", "clear_graph", {"USE_ENTITY_CACHE": "false"})
    config = runner.RunnerConfig(
        port=8001,
        prefix="P5DSHADOW",
        count=1,
    ...
    monkeypatch.setattr(runner, "seed_alerts", lambda *_args, **_kwargs: {"seeded": True})
    monkeypatch.setattr(runner, "verify_queue", lambda *_args, **_kwargs: {"queue_visible": True, "missing_ids": []})
    monkeypatch.setattr(
        runner,
        "capture_workloads",
        lambda *_args, **_kwargs: {
            "unique_once": {
                "summary": runner.latency_stats([0.1], request_count=1),
                "captures": [],
                "failures": [],
    ...
    monkeypatch.setattr(runner, "check_health", lambda *_args, **_kwargs: {"ok": True})
...
def test_phase_dependencies_block_later_phases_in_all(tmp_path, monkeypatch):
    ...
    assert results["summary"]["failed_phase"] == "false_baseline"
    assert results["summary"]["blocked_phases"] == {
        "true_compare": "false_baseline_failed",
        "proof_250": "true_compare_blocked",
    }
    ...
    summary = runner.build_final_summary(
        {
            "false_baseline": {
                "status": "FAIL",
                "graph": "false_graph",
                "error_type": "RouteValidationError",
                "error": "failed to clear stale backend on port 8001; final_port_open=True",
            },
            "true_compare": {
                "status": "BLOCKED",
                "graph": "true_graph",
                "blocked_reason": "false_baseline_failed",
            },
            "proof_250": {
                "status": "BLOCKED",
                "graph": "proof_graph",
                "blocked_reason": "true_compare_failed",
            },
        }
    )

    assert summary["status"] == "FAIL"
    assert summary["failed_phase"] == "false_baseline"
    assert summary["failed_error_type"] == "RouteValidationError"
    assert "failed to clear stale backend" in summary["failed_error"]
    assert summary["blocked_phases"] == {
        "true_compare": "false_baseline_failed",
        "proof_250": "true_compare_failed",
    }
