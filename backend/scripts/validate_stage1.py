...
d = json.load(open("data/soc_multihop_stage1.json"))
S = d["scenarios"]
fails, warns = [], []
def chk(cond, msg):
...
CATS = {"credential_access", "lateral_movement", "data_exfiltration",
        "insider_threat", "cloud_infrastructure", "malware_execution"}
FACTORS = {"privileged_identity_context", "asset_criticality", "threat_intel_enrichment",
           "time_anomaly", "pattern_history", "device_trust"}

KIND = {"SOC-MH-001": "score_keyed", "SOC-MH-002": "score_keyed",
        "SOC-MH-008": "score_keyed", "SOC-MH-013": "score_keyed",
        "SOC-MH-004": "content_keyed", "SOC-MH-009": "content_keyed",
        "SOC-MH-005": "prerequisite", "SOC-MH-003": "prerequisite"}
...
for stem in KIND:
    ...
    chk(len(acts) >= 4, f"{stem}: only {len(acts)} distinct actions {sorted(acts)}")
...
r50 = [x for x in S if x["scenario_id"].startswith("SOC-CTRL-RHO50")]
...
need = {0.30: 3, 0.50: 5, 0.70: 5, 0.90: 3, 1.00: 2}
