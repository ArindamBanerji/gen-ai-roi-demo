...
Factor order mirrors SOC_FACTORS / get_factor_computers():
  0 travel_match             — graph traversal (User, TravelRecord)
  1 asset_criticality        — graph traversal (Alert, Asset, DataClass)
  2 threat_intel_enrichment  — graph traversal (ThreatIntel, Alert)
  3 pattern_history          — graph traversal (Decision, Alert)
  4 time_anomaly             — property read (Alert) [TD-014]
  5 device_trust             — property read (Alert) [TD-015]
...
class ProvenanceService:
    ...
    def build_provenance(
        ...
        factor_names  : ordered list of factor names (6 SOC factors)
    ...
    async def get_provenance_from_graph(
        ...
        category   = resolve_alert_category(alert_type) if alert_type else ""
