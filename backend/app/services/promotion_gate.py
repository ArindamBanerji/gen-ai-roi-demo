...
async def evaluate_promotion(variant_id: str, graph_client) -> PromotionResult:
    ...
    paired_outcomes = batch_stats.get("paired_outcomes")
    bootstrap = paired_bootstrap_test(paired_outcomes or [])
    if not bootstrap["passed"]:
        return PromotionResult(
            "continue" if bootstrap["reason"] == "insufficient_samples" else "reject",
            "paired bootstrap gate failed: " + bootstrap["reason"],
            {
                "total": total,
                "win_rate": win_rate,
...
def _paired_outcomes(events: list[dict[str, Any]]) -> list[dict[str, bool]]:
    outcomes: list[dict[str, bool]] = []
    for event in events or []:
        if event.get("event_type") != SHADOW_RESULT:
            continue
        metadata = event.get("metadata") or {}
        raw_outcomes = metadata.get("paired_outcomes")
        ...
        for outcome in raw_outcomes:
            ...
            if isinstance(baseline, bool) and isinstance(candidate, bool):
                outcomes.append({"baseline": baseline, "candidate": candidate})
    return outcomes
...
def paired_bootstrap_test(
    outcomes: list[dict[str, bool]],
    *,
    min_n: int = MIN_PAIRED_SAMPLES,
    fpr_threshold: float = BOOTSTRAP_FPR_THRESHOLD,
    resamples: int = BOOTSTRAP_RESAMPLES,
    seed: int = 42,
) -> dict[str, Any]:
    """Test candidate-vs-baseline paired improvement with a one-sided bootstrap.

    The null distribution is generated by sign-flipping paired differences,
    preserving each comparison's pairing while testing whether the observed
    improvement could be explained by chance. ``fpr_threshold`` is alpha.
    """
    differences = [
        float(outcome["candidate"]) - float(outcome["baseline"])
        for outcome in outcomes
        if isinstance(outcome, dict)
        and isinstance(outcome.get("baseline"), bool)
        and isinstance(outcome.get("candidate"), bool)
    ]
    sample_count = len(differences)
    evidence: dict[str, Any] = {
        "method": "paired_bootstrap_sign_flip",
        "n": sample_count,
        "min_n": min_n,
    ...
    for _ in range(resamples):
        ...
        if null_mean >= observed_delta:
            at_least_observed += 1
    p_value = (at_least_observed + 1) / (resamples + 1)
    evidence.update(
        {
            "observed_delta": round(observed_delta, 6),
            "p_value": round(p_value, 6),
            "passed": observed_delta > 0 and p_value < fpr_threshold,
            "reason": "passed" if observed_delta > 0 and p_value < fpr_threshold else "not_significant",
        }
    )
    return evidence
...
async def _get_shadow_batch_stats(variant_id: str, graph_client) -> dict[str, Any]:
    try:
        ...
        paired = _paired_outcomes(events or [])
        if paired:
