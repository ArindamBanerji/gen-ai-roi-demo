...
import { circuitBreakAuthority, fetchAuthorityLadder, fetchLearningControlRoom, initializeFrozenComparison } from '../lib/api'
...
export function LearningControlRoom() {
  ...
  const [authority, setAuthority] = useState<AnyRecord | null>(null)
  const [breaking, setBreaking] = useState<string | null>(null)

  const load = () => {
    void fetchLearningControlRoom().then(setData).catch((err: Error) => setError(err.message))
    void fetchAuthorityLadder().then(setAuthority).catch(() => setAuthority(null))
  }
  useEffect(() => { void load() }, [])
  ...
  const circuitBreak = async (category: string) => {
    setBreaking(category)
  ...
  return <section className="space-y-5 rounded-xl border border-cyan-800/60 bg-slate-950/80 p-5 shadow-lg" data-testid="learning-control-room-panel" aria-label="Learning control room">
    ...
    <div className="grid gap-5 lg:grid-cols-2">
     ...
     <div className="rounded-lg border border-amber-800/60 bg-slate-900 p-4"><div className="mb-3 flex items-center justify-between"><h3 className="font-medium text-white">Authority ladder</h3><span className="text-xs text-slate-400">per alert class · RED veto active</span></div>{authority?.categories ? <div className="grid gap-2 md:grid-cols-2">{ensureArray<AnyRecord>(authority.categories).map(item => <div key={String(item.decision_class)} className="flex items-center justify-between rounded border border-slate-800 px-3 py-2"><div><p className="text-sm text-slate-200">{item.decision_class}</p><p className="text-xs uppercase tracking-wide text-amber-300">{item.authority}</p></div><button onClick={() => void circuitBreak(String(item.decision_class))} disabled={breaking === item.decision_class || item.authority === 'circuit-broken'} className="rounded border border-red-800 px-2 py-1 text-xs text-red-300 disabled:opacity-50">{breaking === item.decision_class ? 'Pausing…' : 'Circuit-break'}</button></div>)}</div> : <p className="text-sm text-slate-400">Authority ladder unavailable.</p>}</div>
