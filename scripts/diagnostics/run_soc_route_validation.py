...
def is_port_open(port: int, *, host: str = "127.0.0.1", timeout_seconds: float = 0.25) -> bool:
    try:
...
def stop_stale_backend(
    ...
    *,
    ...
    if not is_port_open(port):
        return {
            "port": port,
            "initial_port_open": False,
            "final_port_open": False,
            "owner_process_ids": [],
    ...
    owner_process_ids: list[str] = []

    if os.name != "nt":
        cleanup_error = RouteValidationError("stale backend cleanup is only implemented on Windows")
    else:
        # Keep the script idempotent: process owners can disappear between discovery and stop.
        # Do not use PowerShell's reserved $PID variable as a loop variable.
        script = (
            f"$ownerProcessIds = @(Get-NetTCPConnection -LocalPort {port} -ErrorAction SilentlyContinue "
            "| Select-Object -ExpandProperty OwningProcess -Unique); "
            "Write-Output ('ownerProcessIds=' + ($ownerProcessIds -join ',')); "
            "if (-not $ownerProcessIds) { exit 0 }; "
        ...
        try:
            ...
            for line in (completed.stdout or "").splitlines():
                if line.startswith("ownerProcessIds="):
                    raw_ids = line.split("=", 1)[1]
                    owner_process_ids = [item for item in raw_ids.split(",") if item]
                    break
    ...
    while time.monotonic() < deadline:
        if not is_port_open(port):
            return {
                "port": port,
                "initial_port_open": True,
                "final_port_open": False,
                "owner_process_ids": owner_process_ids,
                "cleanup_attempted": True,
                "cleanup_returncode": completed.returncode if completed else None,
                "cleanup_stdout": (completed.stdout or "") if completed else "",
                "cleanup_stderr": (completed.stderr or "") if completed else "",
    ...
    final_port_open = is_port_open(port)
    reason = {
        "port": port,
        "final_port_open": final_port_open,
        "timeout_seconds": timeout_seconds,
        "owner_process_ids": owner_process_ids,
        "cleanup_returncode": completed.returncode if completed else None,
        "cleanup_stdout": (completed.stdout or "").strip() if completed else "",
        "cleanup_stderr": (completed.stderr or "").strip() if completed else "",
        "cleanup_error_type": type(cleanup_error).__name__ if cleanup_error else None,
        "cleanup_error": str(cleanup_error) if cleanup_error else None,
    }
    raise RouteValidationError(
        "failed to clear stale backend on port "
        f"{port}; final_port_open={final_port_open}; timeout_seconds={timeout_seconds}; "
        f"owner_process_ids={owner_process_ids}; cleanup_returncode={reason['cleanup_returncode']}; "
        f"stdout={reason['cleanup_stdout']!r}; stderr={reason['cleanup_stderr']!r}; "
        f"cleanup_error={reason['cleanup_error_type']}: {reason['cleanup_error']}"
    )
...
def build_final_summary(results: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for phase_name in (PHASE_FALSE, PHASE_TRUE, PHASE_PROOF):
        payload = results.get(phase_name)
        if not isinstance(payload, Mapping):
            continue
        if payload.get("status") == "BLOCKED":
            blocked_phases[phase_name] = str(payload.get("blocked_reason") or "blocked")
            continue
        if payload.get("status") != "PASS" and failed_phase is None:
            failed_phase = phase_name
            failed_error_type = payload.get("error_type")
            failed_error = payload.get("error")
    if failed_phase is None and isinstance(comparison, Mapping) and comparison.get("status") == "FAIL":
        failed_phase = "comparison"
        failed_error_type = "ComparisonFailure"
        failed_error = ", ".join(str(item) for item in comparison.get("failed_checks", [])) or "comparison failed"
    return {
        "status": "PASS"
...
def run(config: RunnerConfig) -> dict[str, Any]:
    ...
    for phase in phase_plan(config):
        ...
        if phase.name == PHASE_PROOF and PHASE_TRUE in results and results[PHASE_TRUE].get("status") != "PASS":
            blocked_reason = (
                "true_compare_blocked"
                if results[PHASE_TRUE].get("status") == "BLOCKED"
                else "true_compare_failed"
            )
            results[phase.name] = blocked_phase_artifact(
                phase,
                config,
