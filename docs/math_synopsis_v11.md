...
safety gate). A conservation law α(t)·q(t)·V(t) ≥ θ_min=0.467 ensures the two levels
compound rather than conflict. [CHANGED v11]
...
| Selector stabilization | — | ~250 decisions (rolling 100-window) | Selector self-test |
...
| **KernelSelector** | **Auto-select correct kernel?** | **4/4 correct. Rolling 100-window. 250 decisions.** | **✅ PASS** [NEW v11] |
...
(α·q·V drops below 0.7× baseline or absolute floor θ_min=0.467), learning freezes
automatically. No centroid updates until GREEN resumes. Prevents continued corruption
during detected degradation episodes. Three-judge consensus.
...
| Bridge A: θ_min calibration | N_half + deployment V | Conservation law threshold | **✅ RESOLVED: θ_min=0.467 (T_max=21)** |
...
at ~250 decisions. CovarianceEstimator collects data (half_life=300 decisions, does not
affect scoring — v7.0 research). [CHANGED v11: added selector + estimator detail]
...
3 strategies), entity resolution (3-pass pipeline, deterministic canonical IDs).
...
| **θ_min = 0.467** | **η=0.05, N_half=14, T_max=21 days. Conservation floor.** | **derive_theta_min()** | **✅ VALIDATED** [NEW v10] |
...
| **θ_min** | **Conservation law floor** | **scalar** | **0.467 (η=0.05, N_half=14, T_max=21 days)** | **✅ v6.0** [NEW v10] |
...
*Mathematical Synopsis v11 | March 21, 2026*
*v6.0 KERNEL + A=4 + REFERRAL ALL SETTLED. ~104 experiments. 478 GAE + 280 SOC tests (~935 total).*
...
*KernelSelector: rolling 100-window, noise_ratio>1.5→Diagonal, ~250 decisions to stabilize.*
*ReferralRules R1-R7: policy VETO, 72.7% DR, 12% FPR. Override learning v6.5 (≥50 positives).*
*SOC A=4 / S2P A=5 — intentional asymmetry.*
*θ_min=0.467 (T_max=21 days). σ_max=0.034. κ*=0.20. LEARNING_ENABLED=False default.*
