...
(6 categories × 5 actions × 6 factors). The system learns at two formally characterized
levels: Level 1 (ProfileScorer centroids — what to decide, convergence rate (1−η)^n,
Borkar 2008) and Level 2 (AgentEvolver variant selection — how to operate, conservative
contextual bandit with four-condition safety gate). A conservation law
α(t)·q(t)·V(t) ≥ θ_min ensures the two levels compound rather than conflict.
[CHANGED v9, v9.1]
...
### §3.1 — Convergence Analysis for Eq. 4b-final [NEW v9.1]

> **Validation status:** Core results confirmed by three frontier LLM judges
> (GPT-4o, Claude Opus, Gemini). Applicable theorems: Borkar (2008) Theorem
> 5.3.1, Kushner & Yin (2003) Theorem 5.1 + Chapter 10.2. The results below
> are known consequences of classical stochastic approximation theory applied
> to the SOC centroid learning setting.

**Setup.** Focus on a single centroid μ[c, a_GT, :] ∈ [0,1]^d being pulled
toward the firm-specific optimum μ* = E[f | category=c, GT_action=a_GT].
Define error e_n = μ_n − μ*.

---

#### Eq. CONV — Mean Error Dynamics
```
E[e_n] = (1 − η)^n × e_0
where e_0 = μ_0 − μ* (initial prior mismatch — expert guess vs firm reality)
```

**Precision:** Under constant step size η, the iterate does NOT converge almost
surely to μ*. It forms a geometrically ergodic Markov chain around the fixed
point with stationary distribution whose spread is O(√η). The mean contracts
as stated; the iterates track a STATIONARY NEIGHBORHOOD, not a point.

**Half-life of mean error:**
```
N_half = ln(2) / ln(1/(1−η)) ≈ ln(2) / η    for small η
```

At η = 0.05: N_half ≈ 14 verified decisions per (c, a) pair.

---

#### Eq. MSE∞ — Steady-State MSE
```
E[‖e_∞‖²] = η / (2 − η) × tr(Σ_f)
```

where Σ_f is the d×d factor covariance matrix for (c, a_GT). The component
recursions decouple: each dimension evolves as an independent scalar SA.
Cross-covariances in Σ_f affect the joint distribution but NOT the scalar
MSEs — only the trace enters. [Three-judge confirmed: Opus verified component
independence; GPT confirmed the Lyapunov equation derivation.]

**IMPORTANT:** Use tr(Σ_f), not d·σ²_f. Per-factor variances differ in
practice: asset_criticality likely has higher variance than device_trust.
tr(Σ_f) = Σ_j σ²_{f,j} handles heterogeneous variances correctly.

At η=0.05, d=6, tr(Σ_f) ≈ 0.24 (estimated from FX-1-PROXY-REAL factor
distributions): E[‖e_∞‖²] ≈ 0.006, or ‖e_∞‖ ≈ 0.08 (~8% of [0,1] range
per component average).

---

#### Eq. N_CONV — Convergence Time to Calibration Neighborhood
```
N_converge(ε) = ln(‖e_0‖ / max(ε, √(η·tr(Σ_f)/(2−η)))) / η
```

Time for mean error to enter steady-state neighborhood. Calendar conversion:
```
T_calendar(c, a) = N_converge / (V × π_c × f_v × π_{a|c})
```

where V = alerts/day, π_c = category fraction, f_v = verification rate,
π_{a|c} = fraction of category c alerts where GT action is a.

---

#### Conditions and Limitations

**[0,1] Projection:** Projection onto [0,1]^d is non-expansive and does not
change the convergence rate for interior points (projection inactive at μ*).
Consequential when μ* is near boundary: effective step size in projected
dimension reduces to η/2. Quantitative condition: projection matters when
‖μ* − ∂[0,1]^d‖ < c√(η·σ²_f). For typical SOC parameters, this affects
the top/bottom 5-10% of factor value range. [Opus: Borkar 2008, Chapter 5.]

**Push-away coupling:** No direct coupling — pull-toward and push-away modify
distinct centroid vectors. Indirect coupling through confusion matrix:
E[Δμ_a] = η_pos × P(a_GT=a) × E[f−μ_a | a_GT=a]
− η_neg × Σ_{b≠a} P(a_pred=a, a_GT=b) × E[f−μ_a | a_pred=a, a_GT=b]

The limiting geometry balances attraction to true members and repulsion from
confused non-members. Generally beneficial (increases inter-centroid separation).
[GPT: margin expansion. Opus: drift formula above.]

**Centroid retirement pathology [Opus finding]:** With η_neg=0.05 and repeated
misclassification for the same (c,a) pair, the wrong centroid can be pushed
far from all data — effectively retiring that action class. Monitor: for each
(c,a), check that ∃ verified decisions (f, a_GT=a) with ‖f − μ[c,a,:]‖ < D_support.
If no recent decision is within D_support, flag as "out of support."

**Non-i.i.d. factors (campaign clustering):** The i.i.d. result is a baseline.
Under temporal correlation (campaign-clustered alerts), effective variance
inflates:
σ²_eff = σ²_f × (1 + 2 Σ_{k≥1} ρ_k)

For AR(1)-like dependence: n_eff ≈ n(1−ρ₁)/(1+ρ₁). The convergence rate
(1−η)^n is preserved in expectation, but variance is inflated by
autocorrelation. [GPT: effective sample size. Opus: Borkar 2008 Chapter 6.]

**Non-stationarity:** Under drifting target μ*(t), tracking error:
E[‖μ_n − μ_n‖] ≈ √(η·tr(Σ_f)/(2−η)) + ‖dμ/dt‖ / η

First term: steady-state noise (decreases with η). Second term: tracking lag
(increases with η). Constant step size is an advantage: the centroid tracks
a drifting target. Decaying step size would freeze the centroid. Monitor:
rolling mean of (f_n − μ_n); non-zero mean indicates distribution shift.

**Validation status:** Eq. CONV, MSE∞, N_CONV confirmed by three judges as
known results of projected constant-step-size stochastic approximation.
Not novel math — novel application to SOC centroid learning.

**Experiment:** B-1 sweep (extends SHIFT-2): η_pos × ‖e_0‖ × σ²_f × N ×
{isolated pull, push-away coupled, boundary targets, temporally clustered,
correlated factors} at 50 seeds. ~20,000 runs. Validates theory matches
...
#### Level 2 Formal Characterization: Conservative Contextual Bandit [NEW v9.1]

> **Validation status:** Framing confirmed by three frontier LLM judges.
> Literature base: Wu et al. (2016) "Conservative Bandits," Kazerouni et al.
> (2017) "Conservative Contextual Linear Bandits." Additional: Pacchiano et al.
> (2021), Amani et al. (2019), Sui et al. (2018) SafeOpt.

AgentEvolver manages K prompt variants {v₁, ..., v_K} per (category, context)
combination. Context x_t = (alert_severity, analyst_experience_tier, time_period).
On each decision, AgentEvolver selects variant v_t, observes composite reward:
```
Eq. R-L2:  r_t = w₁(phase) × accept_t + w₂(phase) × speed_t + w₃(phase) × outcome_t
```

Phase-gated weights (phase transitions by external milestones, not reward values):
- Phase 1 (Adoption, weeks 0-4): w₁=0.4, w₂=0.3, w₃=0.3
- Phase 2 (Calibration, weeks 4-12): w₁=0.2, w₂=0.2, w₃=0.6
- Phase 3 (Optimization, 12+ weeks): w₁=0.1, w₂=0.2, w₃=0.7

At phase transitions, inflate posterior variance by factor κ to allow
unlearning of prior-phase bias (Bayesian bridging with forgetting factor).

**Eq. GATE-L2 — Four-Condition Promotion Gate:**

A candidate variant v is promoted to default for context x only when ALL
four conditions pass:
```
(1) Superiority:        P(μ_v(x) > μ_baseline(x) + Δ_min | data) > 1 − α
(2) Correctness floor:  outcome_quality(v) ≥ outcome_quality(baseline)
(3) Conservation law:   α(t) · q(t) · V(t) ≥ θ_min     (see Eq. CL below)
(4) Variance stability: σ²_{r,v} ≤ σ²_{r,baseline} × (1 + δ)
```

Gate (1): one-sided Bayesian (Thompson Sampling preferred over frequentist
for rare categories — maintains wider posteriors, naturally handles
exploration). For frequentist fallback: N_gate ≈ (z_α + z_β)² × 2σ²_r / Δ²_min.
At σ_r=0.3, Δ_min=0.05, α=0.05, β=0.20: N_gate ≈ 445 per arm (one-sided).
[Opus correction from 565: one-sided, not two-sided.]

Gate (2): prevents optimizing acceptance at the expense of decision quality.

Gate (3): ensures Level 2 optimization does not cannibalize Level 1's
learning signal. See Eq. CL below.

Gate (4): prevents deployment of variants with unpredictably volatile
outcomes, even if mean is better. Catches adversarial drift where attackers
exploit Level 2's optimization target. [Gemini addition.]

**Eq. CL — Conservation Law (Compounding Safety Constraint):**
```
...
θ_min = minimum informative signal rate for Level 1
```

**Connection to Level 1:** θ_min ≥ N_half / T_window per active (c,a) pair,
where N_half = ln(2)/η from Eq. CONV (§3.1). Level 2 must maintain enough
corrections for Level 1's convergence half-life. As graph matures and N_half
decreases (§10.1), θ_min also decreases — the conservation law becomes EASIER
to satisfy. This is the formal expression of two-level compounding.

**Automation complacency failure mode [all three judges]:**
Level 2 selects high-acceptance variant
→ α(t) decreases (fewer overrides)
→ Level 1 receives fewer correction signals
→ q(t) may decrease (remaining overrides are from autopilot, not expertise)
→ α(t)·q(t)·V(t) drops below θ_min
→ Level 1 improvement stalls → compounding fails

The conservation law (Gate 3) prevents this by rejecting any variant that
pushes the signal rate below θ_min, regardless of acceptance improvement.

**Level 2 adversarial poisoning [Gemini finding]:** A malicious analyst who
consistently accepts bad recommendations on a specific variant can force
promotion of a vulnerable framing. Mitigation: Gate 2 (correctness floor)
catches this if outcome verification is reliable. Additional: EXP-L2-POISON
(planned, analogous to Level 1's EXP-S2-REPRO).

**Drift monitoring:** CUSUM control chart on current default variant's
composite reward. When signaled: flag context for re-evaluation, revert
to baseline, re-enter evaluation with updated priors.

**Regret bound (Kazerouni et al. 2017):**
Regret(T) ≤ O(√(K·d_context·T·ln T)) + K × N_gate × Δ_gap + phase_transition_cost

At K=5, dominant category: full evaluation in ~34 days at 65 verified/day.
Medium categories: require hierarchical sharing across related categories
to achieve feasible evaluation timelines.
...
### §10.1 — Graph-Dependent Convergence [NEW v9.1]

> **Validation status:** Base convergence math confirmed by three judges
> (§3.1). Graph-dependent formulation reviewed by Gemini ("intuition
> brilliant, noise model needs ρ adjustment"). The ρ-adjusted effective
> source equation below incorporates Gemini's correction.

The convergence equations in §3.1 treat Σ_f as fixed. In practice, the
factor covariance Σ_f depends on graph richness — because FactorComputers
TRAVERSE the graph to produce factor values. A richer graph produces more
informative factors.

**Eq. σ²_G — Graph-Dependent Factor Noise:**
```
σ²_{f,j}(G) ≈ σ²_{f,j,base} / N_eff,j(G)
where N_eff,j = N_sources,j / (1 + ρ_j(N_sources,j − 1))
```

- N_sources,j(G): number of data sources contributing to factor j at graph
  maturity G
- ρ_j: average cross-source correlation for factor j
- [Gemini correction]: at ρ=0.8, adding a second SIEM reduces variance by
  ~11%, not 50%. Security sources are highly correlated (often different
  views of the same telemetry).

**Three mechanisms of graph-dependent convergence:**

| Mechanism | How Graph Maturity Helps | Effect on N_converge |
|---|---|---|
| Multi-source noise averaging | More corroborating sources → lower σ²_f (ρ-adjusted) | Steady-state neighborhood shrinks → lower ε floor |
| Entity resolution | Cross-source identity linking → richer per-alert context | ‖e_0‖ decreases (better initial alignment) |
| ThreatIndicator accumulation | Persistent IOC memory → enriched threat_intel factor | σ²_{threat_intel} decreases over time |

**Graph-dependent convergence time:**
N_converge(G) = ln(‖e_0(G)‖ / √(η·tr(Σ_f(G))/(2−η))) / η

Both ‖e_0(G)‖ (initial mismatch) and tr(Σ_f(G)) (factor noise) decrease
with graph maturity. N_converge decreases → the system calibrates faster
the richer the graph becomes.

**Connection to temporal compounding exponent γ:**
If N_converge(G(t)) decreases with time t (because graph richness G(t) grows),
then effective learning speed increases with time — which is the mechanism
for γ>1 in the moat equation I(n,t) ~ O(n^2.11 × t^γ). [Gemini: "The
mapping is legitimate. γ>1 is the result; Bridge B is the engine."]
This provides mechanism-level grounding for EXP-G1. Whether γ>1 holds
empirically is still PENDING EXP-G1 — the mechanism argument does not
substitute for measurement.

**Secondary virtuous loop [Gemini finding]:** As Level 1 improves → alert
volume for analysts drops → analyst fatigue decreases → override quality
q(t) increases → Level 1 gets better correction signal → Level 1 improves
further. This is a third compounding mechanism beyond Levels 1 and 2.

**Three regimes of graph enrichment:**

| Regime | When | N_converge Effect |
|---|---|---|
| Early (sources connecting) | Weeks 1-4 | Drops rapidly |
| Mid (ThreatIndicator accumulating) | Months 1-6 | Drops steadily |
| Mature (graph saturating) | 6+ months | Flattens — diminishing returns |

**Experiment:** B-1 Phase B: simulate graph maturity levels G₁ (single
source) through G₄ (2 SIEMs + entity resolution + 400 ThreatIndicators +
1000 prior decisions) with ρ-adjusted noise. Measure N_converge(G)/N_converge(G₁).
Success: N_converge(G₄) < N_converge(G₁) by ≥30%.

Phase C (temporal test): on 5,000-decision simulation, plot per-window
N_converge. If rate increases with graph accumulation → mechanism-level
evidence for γ>1.
...
| Minimum verification rate floor | Three monitored conditions replace single f_min: (1) coverage per (c,a) ≥ N_min, (2) drift consistency (rolling mean f−μ ≈ 0), (3) α(t)·q(t)·V(t) ≥ θ_min (conservation law). Self-consistent fixed point analysis required for theoretical f_min — deferred to paper. | Core compounding claim, first customer | B-2 Phase A simulation + B-2 Phase B shadow data [CHANGED v9.1] |
| Segmented accuracy by alert type | Realistic L2 margin distribution | Competitive claims, CISO demo | FX-1-PROXY-REAL |
| Bridge A: Level 2 poisoning | EXP-L2-POISON design | v6.0 Level 2 safety | EXP-L2-POISON (planned) |
| Bridge B: ρ per source pair | Empirical source correlation | σ²_G precision | Shadow mode cross-source analysis |
| Bridge A: θ_min calibration | N_half from §3.1 + deployment V | Conservation law threshold | PROD-1 (κ*) + V measurement |
...
| Verification rate health | Compounding requires feedback signal. Three conditions replace single f_min: (1) coverage, (2) drift, (3) conservation law. | Three conditions monitored; conservation law in Eq. GATE-L2 | ⚠️ Partially characterized [CHANGED v9.1] |
| N3 endogenous loop | Known calibration-feedback risk | **No intervention designed** | ⚠️ Known risk |
| Conservation law: α(t)·q(t)·V(t) ≥ θ_min [NEW v9.1] | Level 2 optimization must not cannibalize Level 1 signal | Eq. GATE-L2 condition (3) in eval gate | ✅ DESIGNED (three-judge validated) |
| Centroid support: each (c,a) within data support [NEW v9.1] | Push-away can retire centroid from data (Opus finding) | Monitor ‖f − μ[c,a,:]‖ < D_support for recent verified decisions | ⚠️ Monitoring needed |
| Variant variance stability [NEW v9.1] | Level 2 variant must not be volatile even if mean is better | Eq. GATE-L2 condition (4): σ²_{r,v} ≤ σ²_{r,baseline} × (1+δ) | ✅ DESIGNED (Gemini addition) |
...
| Σ_f | Factor covariance matrix | (d, d) | Positive semi-definite. Use tr(Σ_f) for MSE. | [NEW v9.1] |
| e_n | Centroid error at step n | (d,) | μ_n − μ* | [NEW v9.1] |
| N_half | Convergence half-life | scalar | ln(2)/η ≈ 14 at η=0.05 | [NEW v9.1] |
| N_converge | Decisions to calibration neighborhood | scalar | Eq. N_CONV | [NEW v9.1] |
| α(t) | Analyst override rate | scalar | [0, 1] — fraction disagreeing | [NEW v9.1] |
| q(t) | Override quality | scalar | [0, 1] — fraction of correct overrides | [NEW v9.1] |
| V_verified | Verified decisions per day | scalar | ℕ | [NEW v9.1] |
| θ_min | Conservation law floor | scalar | ≥ N_half / T_window per (c,a) | [NEW v9.1] |
| ρ_j | Cross-source correlation for factor j | scalar | [0, 1] — ρ=0.8 typical for SIEMs | [NEW v9.1] |
| N_eff | Effective independent sources | scalar | N/(1+ρ(N−1)) | [NEW v9.1] |
| K | Number of prompt variants (Level 2) | scalar | 3-5 recommended | [NEW v9.1] |
| w₁,w₂,w₃ | Level 2 reward weights (phase-gated) | scalars | Sum to 1.0 | [NEW v9.1] |
| Δ_min | Level 2 promotion threshold | scalar | 0.05 default (5pp improvement) | [NEW v9.1] |
| N_gate | Samples per arm for promotion | scalar | ≈445 (one-sided, α=0.05, β=0.20) | [NEW v9.1] |
| D_support | Centroid support radius | scalar | Flag when centroid leaves data support | [NEW v9.1] |

---

*Mathematical Synopsis v9.1 | March 17, 2026*
*v5.0 TAGGED. 14 experiments + 50-seed realistic validation + SHIFT-2 + DISC-1 (Mar 15).*
*v9.1: Three-judge validated mathematical bridges incorporated (GPT-4o, Claude Opus, Gemini).*
*NEW: §3.1 Convergence Analysis (Eq. CONV/MSE∞/N_CONV — Borkar 2008, Kushner & Yin 2003).*
*NEW: Level 2 formalized as conservative contextual bandit (Eq. GATE-L2, Eq. CL — Wu et al. 2016).*
*NEW: §10.1 Graph-dependent convergence with ρ-adjusted effective sources (Eq. σ²_G).*
*NEW: Conservation law α(t)·q(t)·V(t) ≥ θ_min — ensures two-level compounding.*
...
*"μ is what you've learned. σ is what you know right now. The good analyst uses both."*
*"The moat is the graph, not the model. The profiles prove it. The two levels compound on it."*
