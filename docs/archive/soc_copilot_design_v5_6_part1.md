# SOC Copilot — Design Document v5.6

**Date:** March 14, 2026
**Version:** 5.6 (v5.5 + ontology verified C=6/A=5, refer_to_analyst implemented,
PROD-3/4/4b complete, CORR-1a routing fixed, η_neg design decision settled)
**Status:** v5.0 TAGGED. v5.5 sprint in progress. 246 GAE tests, v0.5.0 tagged.
111 SOC tests (was 78 at v5.0). 28 experiments complete. GATE-OP PASSED.
**Repository:** soc-copilot (proprietary)
**Theme:** "Your tools, our decisions. Your environment, your intelligence."
**Companion repos:**
- graph-attention-engine (Apache 2.0, 246 tests, v0.5.0 tagged). Design: `gae_design_v9.md` (outputs)
- ci-platform (Apache 2.0, v5.0+). Design: `ci_platform_design_v5_1`
- cross-graph-experiments (25 experiments, 100+ charts). Catalog: `experiments_catalog_v8` (Parts 1/2/3, outputs)

**Git remotes:**
- SOC copilot: git@github.com:ArindamBanerji/gen-ai-roi-demo.git (v5.0-dev branch, tag v5.0)
- GAE library: graph-attention-engine (tag v0.5.0, 246 tests)
- Experiments: git@github.com:ArindamBanerji/cross-graph-experiments.git

> **This document absorbs and supersedes** all prior soc_copilot_design versions (v1 through v5.2). See Appendix C.
> GAE-specific content lives in `gae_design_v9`. Platform content lives in `ci_platform_design_v5_1`.
...
> **Changes from v5.5 → v5.6 (March 14, 2026 — Ontology Expansion + Calibration):**
>
> **(1) CRITICAL: Category names corrected throughout.** Prior doc listed C=5 with names
> "travel_anomaly" and "insider_behavioral" that never existed in code. Actual code has
> C=6: [credential_access, threat_intel_match, lateral_movement, data_exfiltration,
> insider_threat, cloud_infrastructure]. All sections corrected.
>
> **(2) refer_to_analyst implemented (A=5).** Tensor (6,4,6)→(6,5,6)=180 values.
> LLM judge consensus centroids. ReferralPolicy service with activation gates.
> 111 SOC tests.
>
> **(3) τ=0.25 bug in simulation fixed.** get_temperature() now returns 0.1.
>
> **(4) CORR-1a routing fix.** ALERT_TYPE_CATEGORY_MAP (20 entries). 68% of alert types
> were misrouting via silent ValueError fallback. Now 100% correctly mapped.
>
> **(5) Alert pool updated.** 5 healthcare alerts removed, 4 cloud_infrastructure + 3
> refer_to_analyst added. Pool now 27 alerts across all 6 categories.
>
> **(6) PROD-3 complete.** Shadow calibration: 71.9% overall [71.1%, 72.7%].
> Per-category θ values replace 0.85 placeholder in §23.4.1.
>
> **(7) PROD-4b resolves η_neg mismatch.** η_neg=1.0 produces catastrophic miscalibration
> (ECE=0.49). η_neg=0.05 produces excellent calibration (ECE=0.026). All 6 categories
> pass 85% gate at product settings. PROD-4 "0/6 below gate" was artifact of wrong η_neg.
>
> **(8) η_neg DESIGN DECISION settled:** η_neg=0.05 canonical for production.
> η_neg=1.0 FORBIDDEN. 20:1 asymmetry lives in consequence weighting, not learning rates.
>
> **(9) §22.3 IKS:** class→module-level functions to match implementation.
>
> **(10) §14 SOCDomainConfig rewritten for verified code.**
...
```
graph-attention-engine (Apache 2.0)     ← numpy-only, zero external deps, 246 tests
        ↑
...
`SOCDomainConfig.get_profile_centroids()` is domain expertise compiled into geometry. The statement "for insider_threat alerts, escalate when asset_criticality is high and pattern_history is low" compiles to `μ[insider_threat, escalate, :] = [0.35, 0.85, 0.60, 0.15, 0.50, 0.30]`. The mathematical engine computes L2 distance with zero domain knowledge — all domain knowledge has been compiled away into Layer 2's geometry.
...
**Separation constraint (permanent):** ProfileScorer.update() has no variant parameter; AgentEvolver promotion signals never update centroids. The two mechanisms write to different state and cannot contaminate each other. This is P16 in `gae_design_v9.md` — architecturally enforced, not configurable.
...
│       │   ├── iks.py                      # compute_iks(), interpret() — module-level functions (v5.5, §22)
│       │   ├── evolver.py                  ✅
│       │   ├── state_manager.py            ✅
│       │   ├── gae_state.py                ✅
...
### 4.4 v5.0 Canonical Numbers (50-seed validated)
...
| Auto-approve coverage | **11.5%** | ±0.70% | Global threshold |
...
> **Hook contract:** Satisfies `DecisionRecord` (Hook 1, gae_design_v9 §11.4). These are the canonical Hook 1 fields. DecisionRecord is also the data source for GATE-R routing accuracy (TD-035). Any change must preserve all DecisionRecord fields.
...
### 10.2 C=6 is Canonical for Production

```
Production SOC categories (C=6) — verified against config.py SOC_CATEGORIES:
  [0] credential_access    — T1078.004, T1110 Brute Force, anomalous login
  [1] threat_intel_match   — T1566.001, CISA KEV matches, IOC hits, phishing, malware
  [2] lateral_movement     — T1021, T1570, privilege escalation, internal scans
  [3] data_exfiltration    — T1048, T1567, data exfil alerts
  [4] insider_threat       — T1567 Exfiltration, anomalous behavior, insider alerts
  [5] cloud_infrastructure — T1048, cloud config, IAM, storage exposure, drift
```

ProfileScorer shape: **(6, 5, 6)** — 6 categories × 5 actions × 6 factors = 180 values.
*Note: C=5 with `travel_anomaly`/`insider_behavioral` was a documentation error. C=6 has been canonical since v5.4. Shape updated from (5,5,6) to (6,5,6). τ=0.1 (V3B validated, ECE=0.036).*
...
## 10.6 v5.5 Scope — "The Product" (Fully Specified)

v5.5 closes all five CISO demo questions. Requirements ordered by demo conversion impact.

---

### Tier 1 — Demo-Blocking (must ship for any sales motion)

#### v5.5-R1: Category-Specific Auto-Approve Thresholds
*Gap closed: G-L3-1, G-L5-4. Closes Q3 (ROI) and Q4 (safety).*

**PROD-4 rerun (March 14, η_neg=0.05):** All 6 categories achieve ≥85% accuracy.
Per-category threshold* (minimum confidence for 85% accuracy):

| Category | threshold* | Coverage at threshold* |
|---|---|---|
| credential_access | 0.720 | Good — passes ≤0.75 |
| data_exfiltration | 0.730 | Good — passes ≤0.75 |
| lateral_movement | 0.790 | Moderate |
| threat_intel_match | 0.810 | Low |
| cloud_infrastructure | 0.820 | Low |
| insider_threat | 0.870 | Very low — by design (high-risk category) |

Margin-based alternative: margin_star ≈ 0.6 for credential_access achieves same
accuracy at better coverage. Recommend hybrid gating for Phase 5.

**CRITICAL:** Original PROD-4 at η_neg=1.0 showed 0/6 categories passing — that was
an artifact of catastrophic miscalibration. See §22.6.

**Fifth action economics:** Moving 35% of alerts from 15-min full review to 3-min
`refer_to_analyst` pre-analysis recovers ~12 analyst-hours per 100 alerts.
...
### Tier 2 — Sales-Blocking (must ship before first customer contract)

#### v5.5-R6: Alert Type → Category Mapping Completion
*Gap: G-L1-1 [CLOSED — March 14, 2026].*

CORR-1a: ALERT_TYPE_CATEGORY_MAP created (20 entries) + resolve_alert_category().
68% of alert types were misrouting to credential_access via silent ValueError
fallback. No mapping function existed. Now 100% mapped with ROUTING_FAILURE logging.
111 tests passing. GATE-R can run after CORR-1b (Neo4j AlertType seeding).
...
**PROD-4b calibrated values (March 14, η_neg=0.05):**
refer_to_analyst confidence floors: credential_access=0.93, threat_intel_match=0.95,
lateral_movement=0.97, data_exfiltration=0.95, insider_threat=0.96,
cloud_infrastructure=0.95. These replace the 0.70 design estimate.

**ReferralPolicy activation gates (backend/app/services/referral_policy.py):**
1. Centroid proximity: refer is top-1 or competitive top-2 (prob ≥ 0.15)
2. Confidence band: 0.30 ≤ max_prob ≤ 0.70
3. Margin gate: top-2 gap ≤ 0.20
4. Category overrides: threat_intel_match (TI>0.50), data_exfiltration
   (asset>0.70 AND time>0.60), insider_threat (pattern>0.70 AND time>0.70)
...
Alert arrives → SituationAnalyzer classifies category c
  ...
  → ProfileScorer: P(a|f,c) = softmax(-||f - μ[c,a,:]||² / 0.1)
  → Category-specific threshold applied                               [R1 NEW]

  Routing:
    Confidence ≥ threshold → AUTO-APPROVE (40% of alerts)            [R1]
    Confidence 0.70–threshold → REFER TO ANALYST (35%)               [R1, R11]
    Confidence < 0.70 → REVIEW REQUIRED (25%)                        [R11]
  ...
  Tab 3 display per alert:
    ...
    Factor breakdown — 6 bars WITH provenance:                        [R2 NEW]
      ...
      pattern_history:         0.78 → "34/44 prior credential_access correctly suppressed"
    ...
    After feedback submitted:                                         [R3 NEW — learning visible]
      "Your feedback updated the credential_access profile.
...
───── TAB 2: RUNTIME EVOLUTION / LEARNING IMPACT (Proof-of-compounding surface — Role 2) ─────
  ...
  Section A — This Decision (anchored from Tab-3 bridge, or most recent event):
...
───── TAB 4: COMPOUNDING ANALYSIS (ROI and autonomy story — Role 2) ─────
  ...
  Auto-approve coverage by category:                                  [R1 NEW]
    ...
    insider_threat:          2%  ← CORRECT — catastrophic risk, near-zero by design
...
───── SHADOW MODE BANNER (first deployment — any new customer) ─────
  ...
  After threshold reached → Shadow Report generated:
    ...
    "Top disagreement category: insider_threat (61%)."
...
- Referral policy: from app.services.referral_policy import should_refer_to_analyst, get_fallback_action
- Alert routing: from app.domains.soc.config import resolve_alert_category, ALERT_TYPE_CATEGORY_MAP
...
- ProfileScorer shape is (C, A, d) = (6, 5, 6) = 180 values.
- C=6 PRODUCTION categories (ORDER IS PERMANENT):
    0: credential_access  1: threat_intel_match  2: lateral_movement
    3: data_exfiltration  4: insider_threat  5: cloud_infrastructure
- Healthcare REMOVED from alert pool (March 14). NOT a SOC category.
- Alert routing: use resolve_alert_category(alert_type) from config.py.
  NEVER treat alert_type as category directly.
- η_neg = 0.05 (symmetric with η). NEVER use η_neg=1.0 in production.
...
- L2 kernel is default (EXP-E1). Dot product is forbidden (EXP-C1: 61% vs 97.89%).

[Actions]
- refer_to_analyst IS a first-class production action — not a fallback or exception handler.
- It has a centroid profile μ[c, refer_to_analyst, :] for every category.
- Confidence floor (default 0.70) is domain-configurable in CalibrationProfile.
- The fifth action changes the auto-approve economics (§10.6-R1). Never skip it.
...
- SIMILAR_CASES_MIN_SIM: Per-category θ from PROD-3 (replaces 0.85 placeholder):
    credential_access=0.787, threat_intel_match=0.745, lateral_movement=0.809,
    data_exfiltration=0.772, insider_threat=0.792, cloud_infrastructure=0.744
...
class SOCDomainConfig:
    ...
    SHAPE NOTE (v5.3 change):
      get_profile_centroids() returns shape (6, 5, 6).
      Previously (5, 4, 6) when refer_to_analyst was not a production action.
      Now (6, 5, 6): 6 categories × 5 actions × 6 factors.
    ...
    def get_actions() -> list[str]:
        ...
        """
        return ["escalate", "investigate", "suppress", "monitor", "refer_to_analyst"]

    @staticmethod
    def get_categories() -> list[str]:
        """C=6 production SOC categories. ORDER IS PERMANENT — bound to centroid axis 0.
        NOTE: Prior drafts listed C=5 with "travel_anomaly" and "insider_behavioral".
        Those were never implemented.
        S2P analog: ["routine_purchase", "high_value_contract", "sole_source",
                     "compliance_sensitive", "emergency_procurement"]
        """
        return [
            "credential_access",     # index 0
            "threat_intel_match",    # index 1
            "lateral_movement",      # index 2
            "data_exfiltration",     # index 3
            "insider_threat",        # index 4
            "cloud_infrastructure",  # index 5
        ]
    ...
    def get_profile_centroids() -> np.ndarray:
        """μ₀: (6, 5, 6) — 6 categories × 5 actions × 6 factors.
        ...
        Shape:
          Axis 0: category  [credential_access, threat_intel_match, lateral_movement, data_exfiltration, insider_threat, cloud_infrastructure]
          Axis 1: action    [escalate, investigate, suppress, monitor, refer_to_analyst]
          Axis 2: factor    [travel, asset, threat_intel, pattern, time, device]
        ...
        credential_access_c0 = np.array([
            # escalate: unconfirmed travel, high-value asset, suspicious timing
            [0.85, 0.70, 0.40, 0.30, 0.65, 0.50],
            # investigate: ambiguous travel signal — check further
            [0.55, 0.50, 0.35, 0.40, 0.50, 0.50],
            # suppress: known traveler, trusted device, no threats
            [0.15, 0.30, 0.10, 0.75, 0.20, 0.85],
            # monitor: low-priority travel anomaly
            [0.40, 0.40, 0.20, 0.55, 0.40, 0.60],
            # refer_to_analyst: borderline travel evidence, judgment needed
            [0.45, 0.45, 0.25, 0.50, 0.45, 0.55],
        ])

        # Category 1: credential_access
        credential_access = np.array([
            # escalate: after-hours, high criticality, IOC match, novel pattern
            [0.15, 0.88, 0.92, 0.10, 0.80, 0.20],
            # investigate: anomalous but no threat intel confirmation
            [0.10, 0.60, 0.40, 0.30, 0.65, 0.40],
            # suppress: known pattern, low criticality, trusted device
            [0.08, 0.20, 0.08, 0.85, 0.15, 0.90],
            # monitor: borderline credential activity — watching
            [0.10, 0.40, 0.25, 0.55, 0.40, 0.55],
            # refer_to_analyst: confidence below threshold, pre-analyzed
            [0.10, 0.50, 0.30, 0.45, 0.50, 0.45],
        ])

        # Category 2: threat_intel_match
        threat_intel = np.array([
            # escalate: active campaign, critical asset, confirmed IOC
            [0.20, 0.90, 0.95, 0.15, 0.50, 0.30],
            # investigate: partial IOC match — worth checking
            [0.15, 0.65, 0.60, 0.25, 0.45, 0.45],
            # suppress: known false positive campaign match
            [0.08, 0.25, 0.12, 0.80, 0.20, 0.70],
            # monitor: weak IOC signal, low-priority campaign
            [0.12, 0.40, 0.35, 0.55, 0.35, 0.50],
            # refer_to_analyst: ambiguous IOC evidence
            [0.15, 0.50, 0.45, 0.40, 0.40, 0.45],
        ])

        # Category 3: insider_threat
        insider_threat_c3 = np.array([
            # escalate: anomalous data access, high criticality, declining pattern
            [0.35, 0.85, 0.60, 0.15, 0.55, 0.30],
            # investigate: single behavioral anomaly — check further
            [0.28, 0.62, 0.40, 0.35, 0.50, 0.45],
            # suppress: consistent historical pattern, low-risk data
            [0.12, 0.15, 0.18, 0.85, 0.15, 0.70],
            # monitor: behavioral flag, watching for escalation
            [0.25, 0.38, 0.30, 0.60, 0.40, 0.50],
            # refer_to_analyst: borderline behavioral signal (HIGH threshold for this cat)
            [0.30, 0.52, 0.35, 0.42, 0.45, 0.45],
        ])

        # Category 4: cloud_infrastructure
        cloud_infrastructure = np.array([
            # escalate: unusual cloud activity, unknown device, no pattern
            [0.15, 0.55, 0.65, 0.18, 0.40, 0.20],
            # investigate: cloud anomaly worth checking
            [0.10, 0.40, 0.40, 0.35, 0.38, 0.38],
            # suppress: routine cloud access, known pattern, trusted device
            [0.08, 0.18, 0.12, 0.82, 0.12, 0.88],
            # monitor: low-risk cloud activity
            [0.10, 0.28, 0.22, 0.65, 0.22, 0.60],
            # refer_to_analyst
            [0.10, 0.38, 0.28, 0.48, 0.32, 0.45],
        ])

        centroids = np.stack([
            credential_access_c0, credential_access, threat_intel,
            insider_threat_c3, cloud_infrastructure
        ])  # shape: (6, 5, 6)
        # NOTE: C=6 requires 6 categories — this stub still uses C=5. Expand to C=6 in SEED-2.

        assert centroids.shape == (6, 5, 6), f"Expected (6,5,6), got {centroids.shape}"
    ...
    def get_category_thresholds() -> dict[str, float]:
        ...
        """
        return {
            "credential_access":    0.720,  # PROD-4b calibrated (March 14, η_neg=0.05)
            "data_exfiltration":    0.730,
            "lateral_movement":     0.790,
            "threat_intel_match":   0.810,
            "cloud_infrastructure": 0.820,
            "insider_threat":       0.870,  # near-zero coverage — correct (catastrophic risk)
        }
    ...
    def get_alert_category_mapping() -> dict[str, str]:
        ...
        Rules:
          1. Every recognized alert_type maps to exactly one of the 6 canonical categories.
        ...
        Note: CORR-1a: ALERT_TYPE_CATEGORY_MAP created (20 entries) + resolve_alert_category().
        68% of alert types were misrouting. See backend/app/domains/soc/config.py.
        """
        return {
            # credential_access (includes geo/login anomalies — T1078, VPN anomalies, GEO_IMPOSSIBLE_TRAVEL)
            "TRAVEL_ANOMALY":           "credential_access",
            "VPN_ANOMALY":              "credential_access",
            "UNUSUAL_LOGIN_LOCATION":   "credential_access",
            "GEO_IMPOSSIBLE_TRAVEL":    "credential_access",
            "CONCURRENT_SESSION":       "credential_access",
            ...
            # insider_threat (T1567, anomalous data access, behavioral alerts)
            "UNUSUAL_DATA_ACCESS":      "insider_threat",
            ...
            # ... ~160 more entries required — domain expert review in SEED-2 sprint
        }
...
**Shape note (v5.6):** mean_drift averages over 30 (c,a) cells (6×5). The 6
refer_to_analyst cells may contribute near-zero drift early on. D_MAX=0.30 was
set for 24 cells (6×4). PROD-1 should validate at (6,5,6). Not blocking.

### 22.3 IKS Service

```python
# backend/app/services/iks.py — Module-level functions, NOT a class.
# Prior design specified InstitutionalKnowledgeScoreService class.
# Implementation uses module-level functions. Implementation is authoritative.
# Usage: from app.services.iks import compute_iks, interpret
# WRONG: iks_service = InstitutionalKnowledgeScoreService()  # does not exist

import numpy as np, json
from pathlib import Path

D_MAX = 0.30  # Pending PROD-1 recalibration for (6,5,6) tensor

_MU_ZERO_PATH = Path(__file__).parent.parent / "data" / "iks_bootstrap_soc.json"

def _load_mu_zero() -> np.ndarray:
    with open(_MU_ZERO_PATH) as f:
        data = json.load(f)
    return np.array(data["mu_zero"])

def compute_iks(centroids: np.ndarray, mu0: np.ndarray, n_decisions: int) -> float:
    """IKS(t) = 100 × min(mean_drift(t) / D_MAX, 1.0)"""
    diffs = centroids - mu0
    per_cell_dist = np.linalg.norm(diffs, axis=-1)
    mean_drift = per_cell_dist.mean()
    return round(100.0 * min(mean_drift / D_MAX, 1.0), 1)

def interpret(iks_score: float) -> str:
    if iks_score < 5: return "Expert priors only. Learning not started."
    elif iks_score < 20: return f"IKS {iks_score}: Early adaptation."
    elif iks_score < 50: return f"IKS {iks_score}: Meaningful adaptation."
    elif iks_score < 75: return f"IKS {iks_score}: Deep adaptation. Switching cost is real."
    else: return f"IKS {iks_score}: Full adaptation."
```
...
Load on startup: `compute_iks() module-level function` reads this file via `_load_mu_zero()`.
...
Once μ₀ is persisted, implement `compute_iks()` module-level function from §22.3.
...
STEP 0B: IKS service
  ...
  If YES: verify compute_iks() module-level function matches §22.3 spec.
  If NO:  implement from §22.3 (~30 lines).
...
class NLTemplateEngine:
    ...
    # 8 templates: one per category, one for refer_to_analyst, one generic

    # Legacy name — maps to credential_access (NOT in render_l1 template_map)
    L1_TRAVEL_ANOMALY = (
    ...
    # NOTE: L1_INSIDER_BEHAVIORAL kept as alias for backward compatibility
    ...
    def render_l1(self, category: str, context: dict) -> str:
        template_map = {
            ...
            "lateral_movement":     self.L1_LATERAL_MOVEMENT,
            "data_exfiltration":    self.L1_DATA_EXFILTRATION,
            "insider_threat":       self.L1_INSIDER_THREAT,
...
**PROD-3 COMPLETE (March 14, 2026).** Per-category θ values:

| Category | θ | N pairs |
|---|---|---|
| lateral_movement | 0.809 | 28,259 |
| insider_threat | 0.792 | 27,846 |
| credential_access | 0.787 | 28,136 |
| data_exfiltration | 0.772 | 27,599 |
| threat_intel_match | 0.745 | 26,236 |
| cloud_infrastructure | 0.744 | 27,719 |

Regime: centroidal synthetic, 50 seeds, noise_rate=0.10, C=6, A=5, τ=0.1.
Use per-category θ, not a single global threshold.
...
> explanations, 6×5, LLM Sonnet scoring all criteria). This version is a human analyst
...
concepts:
  ...
  - name: lateral_movement_trend
    description: "Lateral movement alert volume and escalation rate over 30 days"
    cypher_template: |
      MATCH (d:Decision)-[:DECIDED_ON]->(a:Alert {situation_type: 'lateral_movement'})
      WHERE a.timestamp > datetime() - duration({days: $days})
...
*SOC Copilot — Design Document v5.6 (Part 1 of 3) | March 14, 2026*
*v5.0 tagged: 246 GAE tests, 111 SOC tests. Ontology verified: C=6, A=5, (6,5,6)=180.*
*PROD-3/4/4b complete. η_neg=0.05 canonical. CORR-1a routing fixed.*
