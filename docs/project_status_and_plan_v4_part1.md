...
# Project Status & Plan v4 — Part 1: Foundation, Current State & Sprint Status

**Version:** 4.0 · Part 1 of 2 · March 21, 2026
**Status:** v5.5 ALPHA COMPLETE. v6.0 kernel + referral architecture SETTLED.
  GAE: 478 tests. SOC: 280 tests. ci-platform: 73 tests. Experiments: ~104.
**Replaces:** project_status_and_plan_v3 (March 10, 2026). All prior versions obsolete.
**Authority:** This document + Part 2 are the execution guide for v6.0.
...
|---|---|
| **Authoritative (current)** | project_status_and_plan_v4 Parts 1+2, math_synopsis_v10, claims_registry_v3_1 (v5 update pending from roadmap session), gae_design_v10.1, soc_copilot_design_v5_5 (3 parts, referral update partial), experiment_reference_catalog_v2 (referral update pending), product_strategy_v2, platform_roadmap_v17 (v19 pending from roadmap session), multivariate_foundation_design_note_v1 |
...
| **Reference (read for context)** | architecture_philosophy_v1_3, ci_platform_design_v5_1, cross_graph_attention_v3, judge_synthesis_v1 |
| **Stale (do not cite)** | Any project_status_and_plan before v4. gae_design before v10. platform_roadmap before v17. experiments_catalog_v8 parts (superseded by master catalog — update pending). **compounding_intelligence_v6 — superseded by v7.** |

---

## 2. Current State (v5.5 Alpha / v6.0 Kernel Architecture Settled)

### Repositories

| Repo | Path | Branch | Tag | Tests | License |
|---|---|---|---|---|---|
| graph-attention-engine (GAE) | `C:\...\graph-attention-engine-v50` | v5.0-dev | v0.5.0 | **478** | Apache 2.0 |
| gen-ai-roi-demo-v4 (soc-copilot) | `C:\...\gen-ai-roi-demo-v4-v50` | v5.0-dev | v5.0 | **280** | Proprietary |
| cross-graph-experiments | `C:\...\cross-graph-experiments` | main | — | ~104 experiments | Internal |
| **ci-platform** | `C:\...\ci-platform` | main | — | **73** | Apache 2.0 |
...
- ci-platform: `C:\Users\baner\CopyFolder\IoT_thoughts\python-projects\kaggle_experiments\claude_projects\ci-platform` (GitHub: ArindamBanerji/ci-platform)
...
| Centroid tensor 5×5×6 = 150 values (n_cat=5, n_act=5, d=6) | ✅ Complete |
...
| Realistic seed data (25 alert types, 6 categories, healthcare domain) | ✅ Complete |
...
### Centroid Tensor (v6.0 Canonical — A=4)
...
Actions (A = 4):
  ...
  2: suppress
  3: monitor
  NOTE: refer_to_analyst removed as scorable action.
...
Factors (d = 6):
  ...
  3: time_anomaly              (NOTE: was index 3 in v5.0 — factor order may vary)
  4: pattern_history
  5: device_trust

Bootstrap: 1,200 synthetic decisions, σ=0.08, seed=42.
Converged=True, drift=0.0097.

Key v6.0 change: A=4 (was A=5). Static accuracy 80.6→90.6%.
Zero dangerous actions with 4-action space.
```
...
This is the single source of truth for execution status. Update this table as items
complete. When starting a new session, find the first row without ✅ and execute it.

| Step | Action | Type | Status |
|---|---|---|---|
| **0a** | Write §17.5 rollback execution semantics | Design doc | ✅ DONE (Mar 11) |
| **0b** | Write §23.5 NL judge rubric | Design doc | ✅ DONE (Mar 11) |
| **0c** | GAE-WIRING-1: CentroidUpdate dataclass | Code (GAE) | ✅ DONE (Mar 12 — 246 tests) |
| **0d** | SOC WIRING-1: centroid_delta_norm + IKS + μ₀ sidecar | Code (SOC) | ✅ DONE (Mar 12) |
| **0e** | VIS-2: Tab-2/3/4 redesign | Code (SOC) | ✅ DONE (Mar 12) |
| **1** | PROD-3 (shadow baseline) | Experiment | ✅ DONE (via PROD-5 + persona sweeps — calibration table superseded by deployment qualification) |
| **2** | PROD-4 (threshold calibration) | Experiment | ✅ DONE (via persona sweeps — per-category thresholds established) |
| **3** | FX-1-PROXY-REAL (realistic L2 margins) | Experiment | ✅ DONE (2,430 IOC records — factor distributions characterized) |
| **4** | §23.4 similar past cases spec | Design doc | ✅ DONE (Mar 11 — θ=0.85 placeholder) |
| **5** | EXP-S2-REPRO Arm 0 (replication) | Experiment | ✅ DONE (poisoning resilience 0.15pp at 20% poison) |
| — | **v5.5 SPRINT** | — | **✅ COMPLETE (alpha)** |
| **S1** | Phase 1: R6 routing + G-L2-3 + G-L4-2 | Code | ✅ DONE — GATE-R: 100% routing accuracy |
| **S2** | GATE-R after Phase 1 | Experiment | ✅ DONE — composite accuracy claim activated |
| **S3** | Phase 2: NL template engine | Code | ✅ DONE — 30 templates (6×5), three layers |
| **S4** | Phase 3: IKS + Chart A correction | Code | ✅ DONE — IKS live, κ* calibrated |
| **S5** | PROD-1 after Phase 3 | Experiment | ✅ DONE — κ* calibrated for IKS |
| **S6** | Phase 4: Shadow Mode + TD-033 | Code | ✅ DONE — checkpoint/rollback, shadow report |
| **S7** | Phase 5: Auto-approve + thresholds | Code | ✅ DONE — 40%+ coverage at ≥85% |
| **S8** | Phases 6-9 | Code | ✅ DONE — provenance, ThreatIndicator, Graph Explorer, bootstrap realism |
| — | **POST-v5.5 WORK (March 19-21)** | — | — |
| **P0** | Asymmetric η fix (η_override=0.01) | Code (GAE) | ✅ DONE — prevents 13-27pp degradation. 309 GAE tests. |
| **B1** | Block 1 demo-ready (attack chain, interventions, compliance, transparency) | Code (SOC) | ✅ DONE |
| **B2** | Block 2 champion-ready (benchmarking, narrative, three claims) | Code (SOC) | ✅ DONE — 246 SOC tests |
| **B3** | Block 3 deploy-ready (SAML, PII, entity resolution, connectors, pipeline) | Code (ci-plat) | ✅ DONE — 73 ci-platform tests |
| **B5A** | Block 5A harnesses (TD-034, PROD-5, B-A Phase B) | Experiments | ✅ DONE |
| **B5B** | Block 5B proxy (9 LLM-judge personas, P0 fix validation) | Experiments | ✅ DONE |
| **PS1** | Phase 1 persona sweeps (6 sweeps, 24 personas, 72 runs) | Experiments | ✅ DONE — product boundaries established |
| **P1V** | Priority 1 validation (V-B3, V-B1, V-CL-RECOVER, 9 personas) | Experiments | ✅ DONE |
| **VHC** | V-HC-CONFIG (mask + diagonal kernel) | Experiments | ✅ DONE — DiagonalKernel rescues healthcare |
| **MV** | V-MV-KERNEL factorial (216 uniform + 144 heterogeneous) | Experiments | ✅ DONE — +13.2pp SOC, +6.8pp S2P |
| **KD** | Kernel deliverables + selector fixes (4 healthcare personas, simplified rule) | Experiments | ✅ DONE — Corr=0.990. Explanation A confirmed. |
| **K1** | Kernel framework (L2Kernel + DiagonalKernel + CovarianceEstimator + KernelSelector) | Code (GAE) | ✅ DONE — 437 GAE tests |
| **FR** | Frozen ROI calculator | Code (SOC) | ✅ DONE |
| **AP** | AMBER auto-pause | Code (GAE) | ✅ DONE — conservation AMBER → freeze learning |
| **S2P** | S2PDomainConfig d=8 | Config (Expts) | ✅ DONE — 5 categories, 5 actions, 8 factors, correlation prior |
| — | **REMAINING v6.0 ITEMS** | — | — |
| **RW** | KernelSelector rolling window fix | Code (GAE) | ✅ DONE — 447 tests |
| **REF-EXP** | 4 referral experiments (A4-DIAG, LEARN, COVERAGE, LAYERED) | Experiments | ✅ DONE — A=4 confirmed, rules ship, conf gate rejected |
| **REF-GAE** | ReferralEngine + OverrideDetector stub | Code (GAE) | ✅ DONE — 447→478 tests (+31) |
| **REF-SOC** | ReferralRules R1-R7 + triage VETO wiring | Code (SOC) | ✅ DONE — 252→280 tests (+28) |
| **D1** | gae_design v10 → v10.1 | Document | ✅ DONE (Mar 21 — 1,752 lines, referral §12.3 added) |
| **D2** | project_status_and_plan v4 | Document | ✅ DONE (this document) |
| **D3** | math_synopsis v10 | Document | ✅ DONE (Mar 21 — referral update pending from roadmap session) |
| **D4** | soc_copilot_design v5_5 update | Document | 🔄 PARTIAL — §22.6 added, 4 mechanical fixes remaining |
| **D5** | experiments_catalog v2 update | Document | 🔲 PENDING — 4 referral experiments to add |
| **D6** | soc_copilot_design v5_5 part2 | Document | 🔲 PENDING — §29.6 referral paragraph |
| **GTM** | Block 4 GTM docs (P10-P14) | Documents | 🔲 PENDING |
| **FE** | Frontend testing | Testing | 🔲 PENDING — zero visual validation done |
| **A1** | P28 DiagonalKernel thresholds | Code (ci-platform) | 🔲 PENDING |
| **A2** | θ_min fix in harness (0.434→0.467) | Code (Expts) | 🔲 PENDING |
| **R2R7** | Neo4j query wiring for R2/R7 | Code (SOC) | 🔲 PENDING — sequence_count, cross_category_count |
...
| **v5.0 post-tag (Mar 12)** | GAE-WIRING-1, SOC WIRING-1, VIS-2 Tab redesign. 246 GAE, 78 SOC. |
| **v5.5 alpha (Mar 15-19)** | Full v5.5 sprint: 18 phases complete. Shadow mode, IKS, NL templates, auto-approve, provenance, ThreatIndicator, Graph Explorer, checkpoint/rollback. Docker Compose VPS deployment. Evidence Ledger. SemanticRegistry. QueryCatalog. GATE-R 100%. PROD-4 thresholds. |
| **v5.5→v6.0 bridge (Mar 19-21)** | A=4 migration (80.6→90.6%). P0 fix: asymmetric η (η_override=0.01). Block 1 (demo-ready), Block 2 (champion-ready), Block 3 (deploy-ready). Phase 1 persona sweeps (24 personas). Kernel architecture: L2Kernel, DiagonalKernel, CovarianceEstimator, KernelSelector. Frozen ROI calculator. AMBER auto-pause. S2PDomainConfig d=8. Referral architecture: 4 experiments (A4-DIAGONAL, REFER-LEARN, REFER-COVERAGE, REFER-LAYERED), ReferralEngine protocol (GAE), ReferralRules R1-R7 (SOC), triage VETO wiring. **478 GAE, 280 SOC, 73 ci-platform tests.** |

### ci-platform Repository (NEW — 73 tests)

| Module | Prompt | Tests | Purpose |
|---|---|---|---|
| redaction/pii_redactor.py | P26 | 10 | 5 regex patterns, 3 strategies (hash/mask/remove) |
| auth/saml.py | P25 | 8 | SP-initiated SAML 2.0 |
| entity_resolution/resolver.py | P27 | 10 | 3-pass pipeline, union-find, deterministic IDs |
| connectors/sentinel.py | P23 | 8 | KQL read, Graph Security write, OAuth2 |
| connectors/splunk.py | P24 | 7 | REST API two-phase search, HEC write |
| connectors/sentinel_writeback.py | P19 | 8 | Decision/provenance/campaign enrichments |
| onboarding/pipeline.py | P28 | 10 | 6-stage pipeline: Extract→Normalize→Redact→Resolve→Load→Compute |
| onboarding/deployment_qualification.py | P28+ | 12 | σ_mean, τ sweep, GREEN/AMBER/RED, remediation |

### Kernel Architecture (Settled March 21, 2026)

| Finding | Evidence | Impact |
|---|---|---|
| DiagonalKernel +13.2pp SOC | V-MV-KERNEL factorial (144 hetero cells) | v6.0 default for noise_ratio > 1.5 |
| DiagonalKernel +6.8pp S2P | V-MV-KERNEL factorial | Same mechanism, different domain |
| DiagonalKernel rescues healthcare | V-HC-CONFIG: +3.7pp vs L2 +0.3pp | Healthcare segment opens at v6.0 |
| Corr(noise_ratio, advantage) = 0.990 | 4 healthcare personas (ratio 1.6-4.6×) | Advantage scales perfectly with noise ratio |
| ShrinkageKernel adds <1pp | Deliverables D2/D3 | Deprioritized to v7.0 |
| Explanation A confirmed | Off-diagonal correlations negligible | Noise ratio alone drives kernel advantage |
| Selector needs rolling window | Cumulative bias: L2 wins early, diagonal wins late | ~250 decisions to stabilize |
| Simplified Phase 2 rule | noise_ratio > 1.5 → diagonal (4/4 correct) | One parameter, no ρ_max needed |
| Noise ceiling kernel-dependent | V-B3 + factorial | L2: σ≤0.157. Diagonal: σ≤0.25 |

### Experiments (~100 Complete)

| Series | Count | Key Findings |
|---|---|---|
| Bridge + Validation (v3.0-v5.0) | 25 | L2=97.89%, b=2.11, τ=0.1, clipping, firewall |
| Block 5A harnesses | 3 | τ=0.08 optimal SOC, convergence ε=0.10, +10.1pp Level 2 |
| Block 5B proxy (9 LLM-judge personas) | 9 | 4 findings → P0 fix (asymmetric η) |
| Phase 1 sweeps (6 sweeps, 24 personas) | 72 runs | Product boundaries: σ, V, q̄, team, enrichment |
| Priority 1 validation | 9 | AMBER safe, recovery conservative, ceiling three-variable |
| V-HC-CONFIG | 5 | Mask halves degradation, DiagonalKernel rescues |
| V-MV-KERNEL factorial | ~360 cells | DiagonalKernel +13pp SOC, +7pp S2P |
| Kernel deliverables + selector fixes | ~20 | Explanation A, simplified rule, 4 HC personas |
| **Referral architecture (4 experiments)** | **~700 runs** | **Rules 72.7% DR / 12% FPR. Conf gate REJECTED. A=4 confirmed (13pp gap).** |
...
|---|---|---|---|---|
| PROD-3 (shadow baseline) | `experiments/prod/prod3_shadow_baseline/` | ✅ Full — catalog Part 3 §14 | Shadow calibration table; cosine distance distribution | §23.4, v5.5-R8 calibration |
| PROD-4 (threshold calibration) | `experiments/prod/prod4_threshold_calibration/` | ✅ Full — catalog Part 3 §14 | threshold*(c) per category; confidence floor | v5.5-R1, 5th action floor |
...
| soc_copilot_design_v5_4 | §22.3 IKS service — correct class → module-level functions | Mar 12 session | 🔵 PENDING |
| soc_copilot_design_v5_4 | §22.5 Add GET /api/soc/centroid-evolution + GET /api/soc/learning-state specs | Mar 12 session | 🔵 PENDING |
| soc_copilot_design_v5_4 | §11.5 Tab-2 Section A — update to purple summary card pattern (not full eval gate trace until live triage) | Mar 12 session | 🔵 PENDING |
| gae_design_v9.md | §9–10 ProfileScorer: add CentroidUpdate dataclass, freeze()/unfreeze(), update() return type | Mar 12 session | 🔵 PENDING |
...
| soc_copilot_design_v5_4 §23.4 | Per-category θ values from PROD-3 cosine distance distribution | PROD-3 completes |
| claims_registry_v2 | FUTURE-12 claim promoted (40%+ coverage) with PROD-4 numbers | PROD-4 completes |
| math_synopsis_v7 §14 | threshold*(c) table with PROD-4 results | PROD-4 completes |
| Any document with "0.70 confidence floor — design estimate" | Replace with PROD-4-derived value | PROD-4 completes |
...
## 10. Parallel Tracks (Updated Status)

| Track | Status | Next Action |
|---|---|---|
| **A: v6.0 Code** | ✅ Kernel + referral architecture complete. 478+280+73 = 931 tests. | P28 DiagonalKernel thresholds, R2/R7 Neo4j wiring, frontend testing |
| **B: Experiments** | ✅ ~104 experiments complete. Referral architecture validated (4 experiments). | V-MV-REGIME and V-MV-INCOMPLETE if time permits (lower priority) |
| **C: Math Paper Update** | 🔵 Staged | Needs DiagonalKernel equation + referral architecture. Blocked by researcher bandwidth. |
| **D: Outreach / GTM** | 🔄 Block 4 GTM docs pending (P10-P14) | DPA template, security posture, VSQ, Loom update |
| **E: Frontend Testing** | 🔴 Zero visual validation done | Every version reveals large frontend errors. Cannot defer past next session. |
| **F: Document Updates** | 🔄 gae_design v10.1 done. soc_copilot partial. 4 docs pending referral pass. | See session_starter_v8 §6 Group A0 |

---

*Project Status & Plan v4.0 · Part 1 of 2 · Last updated March 21, 2026*
*v5.5 alpha complete. v6.0 kernel + referral architecture settled.*
*Four repos: GAE 478, SOC 280, ci-platform 73, experiments ~104. Total: ~935 tests.*
*A=4 actions (escalate, investigate, suppress, monitor). C=6. d=6. Tensor (6,4,6)=144.*
*Referral routing: ReferralRules R1-R7 (72.7% DR, 12% FPR). Confidence gate is action routing only.*
*Three referral phases: Rules (v6.0) → OverrideDetector (v6.5, ≥50 positives) → Retrain (v7.0).*
*η_confirm=0.05, η_override=0.01 (P0 fix). θ_min=0.467. AMBER auto-pause.*
*DiagonalKernel default for noise_ratio > 1.5. L2 fallback. ShrinkageKernel deprioritized to v7.0.*
*Next: Document fixes (5 docs need referral pass), P28 thresholds, R2/R7 wiring, frontend testing, GTM.*
*"The confidence gate catches uncertainty. The rules catch policy. The override learning catches what policy missed."*
